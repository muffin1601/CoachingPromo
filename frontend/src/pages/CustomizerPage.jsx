...
const CustomizerPage = () => {
  ...
  const saveCurrentViewState = () => {
    ...
    if (!canvas) return;
    const json = canvas.toJSON(['id', 'customPart']); // save extra props
    ...
    if (canvas) {
      if (newState) {
        canvas.loadFromJSON(newState, () => {
          // Ensure the mainGroup reference is maintained
          canvas.mainGroup = canvas.getObjects().find(obj => obj.type === 'group');
          applyGlobalColors(canvas);
          canvas.renderAll();
        });
      } else {
        // If no saved state, ensure colors are still applied to fresh canvas
        setTimeout(() => {
          applyGlobalColors(canvas);
        }, 100);
      }
  ...
  useEffect(() => {
    if (!isPreviewOpen) {
      // Rebind canvas to restore interactivity
      const canvas = canvasRef.current;
      if (canvas) {
        canvas.renderAll();
        canvas.calcOffset(); // ensure proper interaction
      }
    }
  }, [isPreviewOpen]);
